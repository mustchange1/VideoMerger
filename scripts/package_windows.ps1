...
$Destination = Join-Path (Split-Path -Parent $Root) "VideoMerger_Final_1.2.4.zip"
$Staging = Join-Path $env:TEMP ("VideoMerger-Package-" + [guid]::NewGuid().ToString("N"))
try {
    New-Item -ItemType Directory -Path $Staging | Out-Null
    robocopy $Root $Staging /E /XD .venv __pycache__ .pytest_cache alignment_models cache /XF *.pyc ffmpeg.exe ffprobe.exe settings.json project_order.json generated_outputs.json | Out-Null
