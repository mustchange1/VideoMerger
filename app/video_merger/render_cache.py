...
def _media_payload(item: MediaInfo) -> dict[str, Any]:
    ...
    return {
        ...
        "is_generated_quote": bool(item.is_generated_quote),
    }
...
def load_cached_alignment(path: Path):
    ...
    words = [WordTiming(**dict(item)) for item in payload.get("words", [])]
    if not words:
        raise ValueError("Cached canonical timeline contains no words")
