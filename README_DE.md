...
- **Neue Phase-2-Defaults:** Cross Dissolve mit 1,0 s (bei kurzen Clips sicher geklemmt) und Music Volume 44 % / Balanced (ca. +6 dB gegenüber 22 %, weiterhin unter Voiceover). Ducking, Limiter, Looping und manuelle Regler bleiben aktiv. Die übrigen Defaults bleiben: Original Audio, Static Phrase, YouTube Landscape, Maximum Quality, End-Padding ≈ 1 s, Quote aus (2,0 s, Clean Editorial), Cut Last Clip, 10 % Stretch, 1,00x Speed. Explizit gespeicherte Transition-/Audio-Werte bleiben erhalten.
...
### Quote-Karte / Flyer-Artwork (optional, stumm)

Die optionale Sektion wird als `Intro → (Übergang) → Quote/Flyer → (Übergang) → Main → (Übergang) → Outro` komponiert. Sie ist standardmäßig deaktiviert und dauert 0,5–5,0 s (Standard 2,0 s). Generierte Text-Quotes behalten alle fünf Stile und die manuellen Typografie-/Layout-Regler. Der Artwork-Modus akzeptiert PNG, JPG, JPEG, WEBP oder eine auswählbare PDF-Seite; PDF-Seiten werden intern mit PyMuPDF in ausreichender Qualität gerastert und die temporäre Rasterdatei nach dem synchronen Export sicher entfernt. Fit, Fill und Crop bewahren das Seitenverhältnis und funktionieren für 16:9, 9:16, 1080p und 4K.

**Quote-/Flyer-Audio bleibt absichtlich stumm**: Voiceover, generierte Musik, Untertitel und Main-Video-Audio werden nicht in die Karte geroutet. Die Karte gelangt nie in die SRT/VTT/Burn-in-Zeitleiste. Die Live-GUI-Vorschau unterstützt Text, Bild-Artwork, ausgewählte PDF-Seiten, Fit-Modus und das gewählte Ausgabeformat mit derselben Geometrie wie der Renderer.
