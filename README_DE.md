...
**Festes 0,7-Sekunden-Ende (nur Bild).** Jeder Short ist exakt so lang wie sein eigenes Voiceover plus `0,7 s` zusätzliches Bildmaterial. Das gesprochene Audio bleibt die maßgebliche Dauer und wird nie verlängert; das Ende enthält keine Sprache, kein Voiceover-Audio und keine Untertitel — die Untertitel-Zeitleiste endet mit dem Voiceover, sodass dort kein Cue dieses oder eines anderen Shorts erscheinen kann. Das zusätzliche Material stammt aus der regulären Timeline-Logik (Clip-Auswahl, Übergänge, Hold/Loop, Chunked Rendering und der Shorts-Pool ohne Ersatz, der das Ende vorab reserviert). Das Long-Form behält das frei einstellbare **Main Video End Padding**.
